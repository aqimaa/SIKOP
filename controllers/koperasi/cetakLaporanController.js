...
exports.cetakLaporan = (req, res) => {
    ...
    db.query(query, [tahun, bulan], (err, results) => {
        ...
        const filePath = path.join(
            __dirname,
            '../../../../views/koperasi/pimpinan/templateLaporan.ejs'
        )
        ...
        ejs.renderFile(filePath, data, (err, html) => {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ message: 'Error rendering template', error: err })
            }
            ...
            pdf.create(html, options).toStream((err, stream) => {
                if (err) {
                    ...
                    return res
                        .status(500)
                        .json({ message: 'Error generating PDF', error: err })
                }

                res.setHeader('Content-Type', 'application/pdf')
                res.setHeader(
                    'Content-Disposition',
                    `attachment; filename=laporan-${tipe}-${tahun}-${bulan}.pdf`
                )
                stream.pipe(res)
            })
        })
